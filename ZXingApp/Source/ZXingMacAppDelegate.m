//
//  ZXingMacAppDelegate.m
//  ZXingMac
//

#import "ZXingMacAppDelegate.h"
#import <ZXingObjc/ZXResult.h>

@implementation ZXingMacAppDelegate
...
- (void) awakeFromNib
	...
	NSLog(@"ZXingMacAppDelegate::awakeFromNib - ENTER");
	#endif
...
- (void)applicationDidFinishLaunching:(NSNotification*) aNotification
	{
	#ifdef __DEBUG_LOGGING__
	NSLog(@"ZXingMacAppDelegate::applicationDidFinishLaunching - ENTER");
	#endif
	...
	if(nil != zxingEngine)
		...
		[zxingEngine setBinaryBool:YES];
		[zxingEngine setLuminanceBool:YES];
		...
		if(nil != captureLayer)
			...
			layertemp = [zxingEngine luminanceLayer];
			...
			layertemp = [zxingEngine binaryLayer];
			...
			[zxingEngine setupZXingCapture:captureDevice
								  delegate:(id<ZXCaptureDelegate>)self
									mirror: mirrorVideoMode];
			[zxingEngine start];
			}
		}
	}
...
- (void) applicationWillTerminate:(NSNotification *)notification
	{
	#ifdef __DEBUG_LOGGING__
	NSLog(@"ZXingMacAppDelegate::applicationWillTerminate - ENTER");
	#endif
...
- (IBAction) windowWillClose:(NSNotification *)notification
	...
	if(((mainWindow == theWindow) || (nil == notification)) && (beenhere == FALSE))
		{
		#ifdef __DEBUG_LOGGING__
		NSLog(@"ZXingMacAppDelegate::windowWillClose - ENTER");
		#endif
...
- (void) dealloc
	{
	#ifdef __DEBUG_LOGGING__
	NSLog(@"ZXingMacAppDelegate::dealloc - ENTER");
	#endif
...
- (void) exitTheApp
	{
	#ifdef __DEBUG_LOGGING__
	NSLog(@"ZXingMacAppDelegate::exitTheApp - ENTER");
	#endif
						...
						if(nil != aCaptureDevice)
							...
							if(nil != currentVideoSourceName)
								...
								if(0 < [currentVideoSourceName length])
									...
									[zxingEngine setupZXingCapture:aCaptureDevice
														  delegate:(id<ZXCaptureDelegate>)self
															mirror: mirrorVideoMode];
									}
								}
...
- (IBAction) captureButtonPressed:(id) sender
	...
	#ifdef __DEBUG_LOGGING__
	NSLog(@"ZXingMacAppDelegate::captureButtonPressed - ENTER");
	#endif
...
- (IBAction) mirrorCheckboxPressed:(id) sender
	...
	#ifdef __DEBUG_LOGGING__
	NSLog(@"ZXingMacAppDelegate::mirrorCheckboxPressed - ENTER");
	#endif
...
- (IBAction) soundsCheckboxPressed:(id) sender
	...
	#ifdef __DEBUG_LOGGING__
	NSLog(@"ZXingMacAppDelegate::soundsCheckboxPressed - ENTER");
	#endif
...
- (IBAction) zxingImagePressed:(id) sender
	...
	#ifdef __DEBUG_LOGGING__
	if (YES == success)
		NSLog(@"ZXingMacAppDelegate::zxingImagePressed - ZXing website access successful.");
	else
		NSLog(@"ZXingMacAppDelegate::zxingImagePressed - ZXing website access failed.");		
	#endif
	}
...
- (void) setupPreferences:(BOOL) forceReset
	{
	#ifdef __DEBUG_LOGGING__
	NSLog(@"ZXingMacAppDelegate::setupPerferences - ENTER - forceReset [%@]", ((YES == forceReset)?@"YES":@"NO"));
	#endif
...
- (ZXCapture*) createZXcapture
	{
	#ifdef __DEBUG_LOGGING__
	NSLog(@"ZXingMacAppDelegate::createZXcapture - ENTER");
	#endif
...
- (void)captureResult:(ZXCapture*)zxCapture 
	...
	NSLog(@"ZXingMacAppDelegate::captureResult - ENTER");
	#endif
	...
	if(nil != inResult)
		...
		if(nil != resultText)
			...
			#ifdef __DEBUG_LOGGING__
			NSLog(@"ZXingMacAppDelegate::captureResult - inResult text[%@]", resultText);
			#endif
...
- (void) presentOverlayForPoints:(CGPoint)point0
	...
	{
	#ifdef __DEBUG_LOGGING__
	NSLog(@"ZXingMacAppDelegate::presentOverlayForPoints - ENTER pt0X[%d] pt0Y[%d] pt1X[%d] pt1Y[%d] pt2X[%d] pt2Y[%d]",
...
- (void)captureSize:(ZXCapture*) inCapture
    ...
    if((0 != [inWidth intValue]) && (0 != [inHeight intValue]))
		{
		NSLog(@"ZXingMacAppDelegate::captureSize - ENTER - inWidth [%d] inHeight [%d]", 
